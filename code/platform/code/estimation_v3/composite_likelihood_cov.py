"""composite_likelihood_cov.py — v2：log-φ 弱信息先验修复版（estimation_v2）。

v2 相对 v1 的唯一行为改动（method_fix 管线，prior_lam=0 时严格退化为 v1）：
  负对数复合似然加入 λ/2·(η−η₀)²，η=log φ（psi[0]），梯度项为 λ(η−η₀)
  （η 即参数本身，链式法则无外因子）。默认 prior_eta0=log(50), prior_lam=0.44
  （log 尺度 sd≈1.5）。新增诊断：phi_on_boundary、converged（区分 maxiter）、
  prior_eta0/prior_lam。multi_start 默认 True 保持。

v1 原文档：存在层协变量的逐类群块复合似然（里程碑 1）。
...
def _neg_loglik_grad(psi, D, W, N, phi_known=None,
    ...
    """负复合对数似然 + log-φ 二次惩罚及解析梯度。

    目标 f = −ℓ(ψ) + λ/2·(η−η₀)²，η=psi[0]=log φ（仅 φ 自由时）。
    ∂f/∂η = −∂ℓ/∂η + λ(η−η₀)——η 即优化坐标本身，无链式外因子。
    ...
    if phi_known is None:
        ...
        if prior_lam > 0.0:
            eta = psi[0]
            pen = 0.5 * prior_lam * (eta - prior_eta0) ** 2
            # grad 为 ℓ 的得分；惩罚 ℓ 的得分为 s − λ(η−η₀)
            grad[0] -= prior_lam * (eta - prior_eta0)
    return -ll + pen, -grad
...
def godambe_covariance_cov(psi, D, W, N, phi_known=None, ridge=1e-10,
    ...
    v2：prior_lam>0 时 A[0,0] += λ（惩罚曲率进入惩罚 M-估计的三明治
    左翼；B 的得分在外层期望值意义下不受惩罚影响，保持逐样本似然得分）。
    """
    A, U = _fisher_and_scores(psi, D, W, N, phi_known)
    if prior_lam > 0.0 and phi_known is None:
        A[0, 0] += prior_lam
    B = U.T @ U
    k = A.shape[0]
...
def fit_composite_cov(D, W, N, phi_known=None, multi_start=True, maxiter=1000,
    ...
    prior_eta0, prior_lam : log-φ 二次惩罚 λ/2·(η−η₀)²（仅 φ 自由时生效；
                prior_lam=0 严格退化为 v1 行为）。

    返回 dict：phi, Gamma (p,K), theta (p,)；se_Gamma、se_beta（Godambe）、
    se_*_naive；loglik（不含惩罚）、loglik_pen（含惩罚目标）、success、
    converged（区分 maxiter）、phi_on_boundary、on_boundary、prior_*、
    Wald 所需全部量。
    """
    ...
    if phi_known is not None:
        ...
        best = None
        for s in starts:
            res = minimize(_neg_loglik_grad, s,
                           args=(D, W, N, None, prior_eta0, prior_lam),
                           method="L-BFGS-B", jac=True, bounds=bounds,
    ...
    # 真收敛判据：success 且非撞 maxiter（L-BFGS-B 撞限时 message 含
    # "ITERATIONS ... LIMIT"；nit 达上限亦记为未真收敛）。phi_known 分支
    # 的 nit 是逐类群子问题迭代之和，不能对 maxiter 直接比较，只看 message。
    if phi_known is None:
    ...
    psi = best.x
    phi, G, theta = _unpack(psi, p, K, phi_known)
    V_god, V_naive, A, B = godambe_covariance_cov(
        psi, D, W, N, phi_known, prior_lam=(prior_lam if phi_known is None
                                            else 0.0))
    ...
    out = {
        ...
        "loglik": composite_loglik_cov(psi, D, W, N, phi_known),
        "loglik_pen": -best.fun,
    ...
    # v2：单独报告 φ 撞界（phi_known 时恒 False）
