...
def three_layer(params, n, p, depths, seed):
    ...
    m_case = _apply_effect(m0, da_mask, prm["effect_size"])
    ...
    theta_tilde = c / S
    Y = np.stack(
        [rng_y.multinomial(int(N[i]), theta_tilde[i]) for i in range(2 * n)]
    ).astype(np.int64)
...
def zinb(params, n, p, depths, seed):
    ...
    *structural* zero), else Y_ij ~ NB(mean = N_i * mtilde_j^{g(i)},
    ...
    prob = size / (size + mu)  # NB(size, prob): mean = size*(1-prob)/prob = mu
    Y = np.where(
        present,
    ...
    truth.update(
        omega=omega, mu=mu, size=size, structural_taxa=struct_taxa,
        designated_structural=sz_mask,
    )
    return Y, truth
...
def beta_binomial(params, n, p, depths, seed):
    ...
    truth.update(
        pi=pi, mu=mu, phi=phi, structural_taxa=struct_taxa,
