"""run_one_v34f.py — v3.4-PILOT：单 (cell, rep) 端到端流水线。
...
用法: python3 run_one_v34f.py --cell 1002 --rep 0 --out out.npz
"""
...
PRIOR_ETA0_DEFAULT = float(np.log(50.0))
PRIOR_ETA0 = PRIOR_ETA0_DEFAULT
LAM_MAIN = 0.44
LAM_RETRY = 1.0
EXCL_THRESHOLD = 0.5
R_REPS_DEFAULT = 20
...
def main():
    ...
    ap.add_argument("--eta0", type=float, default=PRIOR_ETA0_DEFAULT)
    ap.add_argument("--r-reps", type=int, default=R_REPS_DEFAULT)
    args = ap.parse_args()
    global PRIOR_ETA0
    PRIOR_ETA0 = args.eta0

    cfg = pd.read_csv(CONFIG_CSV)
    row = cfg[cfg.cell_id == args.cell].iloc[0]
    seeds = np.random.SeedSequence(int(row["seed"])).spawn(args.r_reps)
