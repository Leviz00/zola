...
} else if (method %in% c("zinq_ln", "zinq_zola")) {
  suppressMessages(library(ZINQ))
  rel <- sweep(Y, 1, pmax(Nlib, 1), "/")
  if (method == "zinq_zola") {
    qh <- as.matrix(read.csv(file.path(dsdir, "qhat.csv"), header = FALSE))
    qh <- pmin(pmax(qh, 1e-6), 1 - 1e-6)
    Wmat <- log(qh / (1 - qh))                       # logit(qhat_j(N_i))
  }
  lnN <- log(pmax(Nlib, 1))
  for (j in seq_len(p)) {
    pv[j] <- tryCatch({
      W <- if (method == "zinq_ln") lnN else Wmat[, j]
      if (sd(W) < 1e-10) W <- W + rnorm(n, 0, 1e-8)   # guard degenerate
      dat <- data.frame(y = rel[, j], X = grp, W = W)
      res <- ZINQ_tests(formula.logistic = y ~ X + W,
                        formula.quantile = y ~ X,
                        C = "X", y_CorD = "C", data = dat)
      as.numeric(ZINQ_combination(res, method = "Cauchy"))
    }, error = function(e) NA_real_)
  }
} else if (method == "locom_zola") {
  suppressMessages(library(LOCOM))
  keep <- scan(file.path(dsdir, "keep_zola.txt"), what = integer(),
               quiet = TRUE)                          # 1-based indices
  Yk <- Y[, keep, drop = FALSE]
  res <- tryCatch(
    locom(otu.table = Yk, Y = grp, C = NULL, fdr.nominal = 0.05,
          seed = 20260819, n.cores = 2, n.perm.max = nperm,
          filter.thresh = 0),
    error = function(e) { message("LOCOM-zola error: ",
                                  conditionMessage(e)); NULL })
  if (!is.null(res)) {
    kept <- colnames(res$p.otu)
    pv[keep[match(kept, colnames(Yk))]] <- as.numeric(res$p.otu)
  }
