"""run_fix_v4.py — SPEC-CH Amendment A3 FINAL conventions rerun (all v4 cells).

Conventions v2 (self-review round 2):
  * detection channel depth variable = REALIZED library (matches real-data
    practice; makes null taxa conditionally null given N' under
    library-coupled generators — the design-depth convention was the leak);
  * intensity offset = median-ratio with 50-taxon reference (median robust
    to 10 DA taxa in the reference; 20-taxon reference was contaminable);
  * library~group diagnostic retained as a reported diagnostic only.
...
from run_pilot_v4 import CELLS as _ALL
FIX_CELLS = dict(_ALL)
FIX_CELLS[2006] = dict(mech="zigdm_like", depth=20000, n=300,
...
if __name__ == "__main__":
    ...
    df.to_csv("/home/claude/ch_smoke/fix2_v4_detail.csv", index=False)
    g = df.groupby("cell")
    ...
    s.to_csv("/home/claude/ch_smoke/fix2_v4_summary.csv")
